...
var wstlType = 'application/prs.wstl+json';
var cType = '';
var cAccept = '';
...
function handler(req, res) {
  ...
  cAccept = req.headers["accept"];
  if(!cAccept || cAccept.indexOf(cjType)!==-1) {
    cType = cjType;
  }
  else {
    cType = cAccept.split(',')[0];
  }
...
function handleResponse(req, res, doc) {
  ...
  if(doc!==null) {
    if(doc.file===true) {
      ...
      rtn = representation(doc.doc, cType, root);
    }
